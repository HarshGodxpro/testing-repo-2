...
# --- Hybrid IMDb + TMDB fetcher (added by assistant) ---
import requests
from imdb._exceptions import IMDbDataAccessError

async def hybrid_get_poster(query, file=None, bulk=False):
    """Try the existing get_poster() first. If it raises or returns falsy, fallback to TMDB.
    Returns a dict compatible with existing template keys used in pm_filter (best-effort).
    """
    # First try existing util (if available)
    try:
        # call original get_poster if it's in globals (it is imported from utils in this file)
        if 'get_poster' in globals():
            try:
                result = await get_poster(query, file=file, bulk=bulk)  # original async call
            except IMDbDataAccessError as ie:
                # allow fallback to TMDB
                result = None
            except Exception:
                # any other error -> treat as no result so fallback to TMDB
                result = None
            if result:
                return result
    except Exception:
        # If something unexpected happens, continue to TMDB fallback
        pass

    # TMDB fallback (synchronous HTTP call using requests)
    try:
        search_url = "https://api.themoviedb.org/3/search/movie?api_key={tmdb}&query={query}".format(tmdb="ec8c79cb7ce985249d2c690a90752c94", query=requests.utils.requote_uri(query))
        ...
        # get details
        details_url = "https://api.themoviedb.org/3/movie/{movie_id}?api_key={tmdb}&append_to_response=credits".format(movie_id=movie_id, tmdb="ec8c79cb7ce985249d2c690a90752c94")
        dresp = requests.get(details_url, timeout=10)
        details = dresp.json()
        # Build a dict compatible with expected template keys (best-effort)
        title = details.get('title') or movie.get('title')
        year = (details.get('release_date') or "")[:4] if details.get('release_date') else None
        rating = details.get('vote_average')
        genres = [g.get('name') for g in details.get('genres') or []]
        poster_path = details.get('poster_path')
        poster = "https://image.tmdb.org/t/p/w780{p}".format(p=poster_path) if poster_path else None
        overview = details.get('overview') or movie.get('overview') or ""
        imdb_id = details.get('imdb_id') or None
        url = "https://www.themoviedb.org/movie/{mid}".format(mid=movie_id)
        # credits
        credits = details.get('credits') or {}
        cast = [c.get('name') for c in (credits.get('cast') or [])][:8]
        crew = credits.get('crew') or []
        director = ', '.join([c.get('name') for c in crew if c.get('job') == 'Director'][:2]) or None
        writer = ', '.join([c.get('name') for c in crew if c.get('job') in ('Writer','Screenplay')][:2]) or None
        ...
        return {
            ...
            'plot': overview,
            'poster': poster,
            'url': url if url else (f"https://www.imdb.com/title/{imdb_id}" if imdb_id else None),
            'imdb_id': imdb_id,
            'cast': cast,
            'director': director,
            'writer': writer,
            ...
            'countries': [p.get('iso_3166_1') for p in (details.get('production_countries') or [])],
            'certificates': None,
            'languages': [l.get('iso_639_1') for l in (details.get('spoken_languages') or [])],
            'producer': None,
            'composer': None,
...
#          text=f"<b>ʜᴇʏ {user} 😍 ,\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ. ʀᴇǫᴜᴇsᴛ ɪᴛ ɪɴ ᴏᴜʀ <a href=https://t.me/+IXXBlPCAiww5NDU1>ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ</a> ᴏʀ ᴄʟɪᴄᴋ ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 👇</b>", disable_web_page_preview=True,
#          reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/Update_Proffsor_Bot")]])
#     )
...
async def next_page(bot, query):
    ...
    imdb = await hybrid_get_poster(search, file=(files[0]).file_name) if settings["imdb"] else None
    ...
    if query.data.startswith("file"):
                        ...
                        await client.send_cached_media(
                            ...
                            reply_markup=InlineKeyboardMarkup(
                                ...
                                [
                                    ...
                                    InlineKeyboardButton("🌿 Mᴏᴠɪᴇ Sᴇᴀʀᴄʜ Gʀᴏᴜᴘ 🥀", url="https://t.me/+IXXBlPCAiww5NDU1")
                                ]
                                ]
                            )
                        )
        ...
        await query.message.edit_text(
            ...
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
                InputMediaPhoto("https://graph.org/file/523b9f39fba066ede1716.jpg")
            )
            reply_markup = InlineKeyboardMarkup(btn)
            await query.message.edit_text(
                text="⋘ Pʟᴇᴀsᴇ Wᴀɪᴛ ⋙"
            ...
            btn = [[
                    ...
                    InlineKeyboardButton("🌼 Mᴏᴠɪᴇ Cʜᴀɴɴᴇʟ🌼", url='https://t.me/Update_Proffsor_Bot')
                ],[
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="start"),
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="telegram.me/PROFE07XH")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            dlt = await message.reply_sticker('CAACAgIAAxkBAAEHabNlEVZ1a9KhLS_pS52uDzHOVYiZjgACYDUAApGbkErePaGX9dep5B4E', reply_markup=InlineKeyboardMarkup(btn))
            search = search.lower()
            find = search.split(" ")
            search = ""
